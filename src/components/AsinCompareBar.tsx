...
import { X, ExternalLink, Star, Minimize2, Maximize2 } from 'lucide-react';
...
export const AsinCompareBar: React.FC<AsinCompareBarProps> = ({
  ...
  if (minimized) {
    return (
      <div className="fixed bottom-4 right-4 z-50 flex items-center gap-2 rounded-xl border border-indigo-100 bg-white px-3 py-2 shadow-xl" data-print-hidden>
        <button
          type="button"
        ...
        >
          ...
          ASIN 对比（{selectedAsins.length}）
        </button>
        <button
          type="button"
          onClick={onClear}
          className="p-1 rounded-lg text-slate-400 hover:bg-rose-50 hover:text-rose-500"
          title="清空"
        >
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-black/5 bg-slate-50">
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            className="text-[10px] text-slate-500 hover:text-indigo-600 transition-colors flex items-center gap-1 rounded-lg px-2 py-1 hover:bg-white"
            title="最小化"
          >
            <Minimize2 className="w-3 h-3" /> 最小化
          </button>
          <button
            onClick={onClear}
            className="text-[10px] text-slate-400 hover:text-rose-500 transition-colors flex items-center gap-1 rounded-lg px-2 py-1 hover:bg-white"
          >
              ...
              <tr key={product.asin} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                <td className="px-3 py-2">
                  <div className="flex items-center gap-2">
                    <ProductImageZoom
                      src={product.image}
                      className="w-8 h-8 rounded object-cover border border-black/5 flex-shrink-0"
                      fallbackClassName="w-8 h-8 rounded bg-slate-100 flex items-center justify-center text-[8px] text-slate-400 flex-shrink-0"
                    />
