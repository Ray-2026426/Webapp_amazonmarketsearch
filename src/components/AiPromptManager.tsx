...
## 数据边界
- 只使用我提供的 ASIN 数据、历史月度数据与页面可见字段。缺失字段统一写「数据缺失」或「当前数据不足，无法判断」。
- 禁止编造评论原文、库存、Buy Box、跟卖数量、广告花费、转化率、利润率、退货率、关键词排名等未提供数据。
- 如果没有 Keepa 原始库存/Offer/Buy Box 历史，不要输出确定性的库存或跟卖结论；只能写「基于销量、BSR、价格波动的运营推断」，并标注置信度。
...
- 对推断类结论添加置信度：高 / 中 / 低。
- 如果历史数据少于 3 个有效月份，在「数据完整性」中说明趋势判断置信度低。

## 报告结构

### 0. 数据完整性与口径说明
用 3-5 条说明本次可用数据范围、缺失数据、分析口径与置信度。
必须说明：本报告基于输入数据生成；若未提供 Keepa 原始字段，则库存、Buy Box、Offer 历史只能作为推断。
...
如果没有库存或 Offer 历史，必须写明「无库存/Offer 原始历史，以下为运营推断」。
输出：
- 是否存在疑似促销拉动
- 是否存在疑似断货或供给波动
- 是否需要关注库存周转
...
用表格输出：
| 类型 | 机会/风险 | 证据 | 影响 | 建议动作 |
至少 3 条机会、3 条风险；证据必须来自输入数据或明确写「待验证」。
...
### 10. 需要补充的数据
列出为了把报告升级到接近 Keepa 完整深度报告，还需要补充的字段：
- Keepa 价格历史、Coupon 历史、BSR 历史
- 库存/Offer/Buy Box 历史
- 评论文本与星级分布
- 关键词流量与广告数据
- 竞品对照 ASIN
