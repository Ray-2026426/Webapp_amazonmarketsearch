...
const DEFAULT_PROMPTS: Omit<AiPromptConfig, 'currentPrompt'>[] = [
  ...
  { id: 'voc_deep_report', name: 'VOC Step3: 深度洞察报告', description: '综合评论数据，生成产品开发深度洞察报告',
    defaultPrompt: '作为资深亚马逊产品开发专家，请基于我提供的评论数据，进行深度用户洞察并输出产品优化方案。结论必须严格基于评论原文，不得凭空捏造。\n\n## 输出要求（请使用 Markdown 格式，方便排版）\n\n请输出 6 个章节，**每个章节使用二级标题** `## 一、用户画像`、`## 二、使用场景`…依次编号到 `## 六、核心卖点`。\n\n各章节具体要求：\n\n- 一、用户画像（≈50字）：描述核心使用人群的身份与关键特征。\n- 二、使用场景（≈50字）：最高频、最具体的使用情境/场合。\n- 三、核心需求（≈100字）：用户最看重的 1–3 个核心价值与功能诉求。\n- 四、主要痛点（≈100字）：现有产品未满足用户的 3–5 个缺陷与抱怨。\n- 五、优化后产品方案（≈300字）：针对痛点的具体改进方案（材质、参数、功能、包装等）。\n- 六、核心卖点（≈200字）：为新方案提炼 2–5 条卖点。\n\n## 排版规范（重要）\n\n- 三、四、六章节请用 **无序列表**（每条以 `- ` 开头），便于扫读。\n- 关键名词、数字、卖点关键词请使用 **`**加粗**`** 强调（不要全段加粗）。\n- 一、二、五章节用一段文字即可；如需对比，可使用 Markdown 表格。\n- 不要在内容里插入 `---` 分隔线、不要使用代码围栏 ``` 包裹。\n- 一律使用简体中文输出。' },
