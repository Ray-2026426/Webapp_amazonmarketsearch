...
export function buildMarketDataQuality(
  ...
  const activeSalesCells = history.reduce(
    (sum, h) => sum + months.filter((m) => (h.history[m]?.sales || 0) > 0).length,
    0
  );
  const activeRevenueCells = history.reduce(
    (sum, h) => sum + months.filter((m) => {
      const d = h.history[m];
      return (d?.sales || 0) > 0 && (d?.revenue || 0) > 0;
    }).length,
    0
  );
  const activePriceCells = history.reduce(
    (sum, h) => sum + months.filter((m) => {
      const d = h.history[m];
      return (d?.sales || 0) > 0 && ((d?.price || 0) > 0 || (d?.revenue || 0) > 0);
    }).length,
    0
  );
  const revenueCoverage = activeSalesCells > 0 ? pct(activeRevenueCells, activeSalesCells) : 100;
  const historyValueCoverage = activeSalesCells > 0 ? Math.min(revenueCoverage, pct(activePriceCells, activeSalesCells)) : 100;
  ...
  if (historyValueCoverage < 70) issues.push({ level: 'warn', message: `有销量月份的销售额/价格完整度 ${fmtPct(historyValueCoverage)}，市场规模和均价判断置信度下降。无销量月份不计入缺失。` });
  ...
  return {
    ...
    metrics: [
      ...
      { label: '历史金额完整度', value: fmtPct(historyValueCoverage), level: levelByCoverage(historyValueCoverage), note: '仅统计有销量月份' },
