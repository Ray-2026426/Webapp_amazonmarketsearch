...
export type McpProviderKind = 'sellersprite' | 'custom';
...
function normalizeProviders(raw: Partial<McpSettings>): McpProviderEntry[] {
  if (Array.isArray(raw.providers) && raw.providers.length > 0) {
    return raw.providers.map((p) => {
      const kind: McpProviderKind = p.kind === 'custom' ? 'custom' : 'sellersprite';
      return {
        id: p.id || uid(),
        name: String(p.name || (kind === 'sellersprite' ? '卖家精灵' : '自定义 MCP')).trim(),
        kind,
...
export function loadMcpSettings(): McpSettings {
  ...
  return {
    secretKey: '',
    mcpUrl: '',
    providers: [createSellerSpriteProvider()],
  };
}
...
export function saveMcpSettings(settings: McpSettings): void {
  const providers = (settings.providers?.length
    ...
    const mcpUrl =
      p.kind === 'sellersprite' && isOfficialSellerSpriteMcpUrl(p.mcpUrl) ? '' : p.mcpUrl.trim();
    return {
      ...p,
      name: p.name.trim() || (p.kind === 'sellersprite' ? '卖家精灵' : '自定义 MCP'),
